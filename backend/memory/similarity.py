...
def euclidean_similarity(v1: List[float], v2: List[float]) -> float:
    ...
    # Map distance to [0, 1] range: 1 / (1 + dist)
    return 1.0 / (1.0 + dist)

def extract_feature_vector(features: Dict[str, float]) -> List[float]:
    """Standardized 5-dimension vector: [accel_x, accel_y, accel_z, vibration, rms]"""
    return [
...
def find_similar_events(
    ...
    """
    ORACLE Memory Similarity Engine:
    Compares the current anomaly behavioral vector against stored historical events.
    Enriches the match with any correlated maintenance records from that period.
    """
    ...
    for ev in events:
        try:
            ...
            # Hybrid similarity weighting: 60% cosine (pattern angle), 40% euclidean (intensity)
            combined_sim = 0.6 * cos_sim + 0.4 * euc_sim
            
            if combined_sim >= min_similarity:
                # Look for maintenance actions near this event (within 7 days = 604800s)
