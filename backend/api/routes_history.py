...
def get_history_endpoint(asset_id: str, limit: int = 100, db: Session = Depends(get_db)):
    """Retrieve chronological timeline uniting Events, Maintenance, and Predictions."""
...
def get_similar_events_endpoint(
    ...
    """
    Query ORACLE Memory: Finds past anomaly episodes matching given sensor feature vector.
    """
    asset = get_asset(db, asset_id)
    if not asset:
        raise HTTPException(status_code=404, detail="Asset not found")
