...
def analyze_endpoint(asset_id: str, request: AnalyzeRequest = None, db: Session = Depends(get_db)):
    """Run full intelligence pipeline on machine: Anomaly Detection + Memory + Decision Engine."""
...
def get_health_endpoint(asset_id: str, db: Session = Depends(get_db)):
    """Fast summary of machine health, status, and failure risk for dashboards."""
    asset = get_asset(db, asset_id)
    if not asset:
        raise HTTPException(status_code=404, detail="Asset not found")
...
def get_predictions_endpoint(asset_id: str, limit: int = 50, db: Session = Depends(get_db)):
    """Get history of ML predictions and failure risks."""
    asset = get_asset(db, asset_id)
    if not asset:
        raise HTTPException(status_code=404, detail="Asset not found")
    import json
