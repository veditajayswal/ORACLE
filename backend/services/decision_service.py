...
def evaluate_decision(
    ...
    Transforms ML metrics + historical memory matches into concrete actionable decisions
    and issues physical commands (e.g. STOP) if critical risk is reached.
    """
    command_issued = None
    matches_models = [HistoricalMatch(**m) for m in historical_matches]
    
    # 1. Base decision logic from risk thresholds
    if failure_risk < settings.RISK_NORMAL_MAX:
        ...
        # Issue physical STOP command to ESP32 relay
        cmd = Command(
            asset_id=asset_id,
            timestamp=time.time(),
            command="STOP",
    ...
    # 2. Enrich reason with ORACLE Memory historical context
